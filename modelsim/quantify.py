...
def parse_txt_to_pseudo_image(
    filepath: str,
    grid_h: int = 496,
    grid_w: int = 432,
    channels: int = 64,
    drop_log_path: str = "sw_dropped_data.txt"
) -> np.ndarray:
    """
    将文本文件解析并散射为伪图。
    支持两类常见格式：

    1) voxel_x voxel_y dim0 dim1 ... dim{C-1}
       总列数 = C + 2

    2) batch_idx voxel_x voxel_y dim0 dim1 ... dim{C-1}
       总列数 = C + 3
    ...
    with open(filepath, 'r') as f, open(drop_log_path, 'w') as f_drop:
        for line_idx, line in enumerate(f):
            ...
            if len(parts) < channels + 2:
                f_drop.write(line_strip + f" (REASON: Too few columns, len={len(parts)})\n")
                continue

            try:
                features = np.array(parts[-channels:], dtype=np.float32)

                prefix = parts[:-channels]

                if len(prefix) == 2:
                    # x y feat...
                    voxel_x = int(float(prefix[0]))
                    voxel_y = int(float(prefix[1]))
                elif len(prefix) >= 3:
                    # batch/index x y feat...
                    voxel_x = int(float(prefix[1]))
                    voxel_y = int(float(prefix[2]))
                else:
                    f_drop.write(line_strip + f" (REASON: Bad prefix len={len(prefix)})\n")
                    continue

            except Exception as e:
                f_drop.write(line_strip + f" (REASON: Parse failed: {e})\n")
                continue
...
if __name__ == "__main__":
    ...
    hw_file = r"E:\verilog\pillarnest\modelsim_pointpillars_32\pfn_layer_out.csv"  
    sw_file = r"E:\mmdetection3d\bev_64d_nonzero_32max.txt" 

    print("\n--- 读取软件特征 ---")
    # 可以通过 drop_log_path 指定软硬件不同的日志文件位置
    sw_fake = parse_txt_to_pseudo_image(sw_file, grid_h=496, grid_w=432, channels=64, drop_log_path="sw_dropped_log.txt")
    print("\n--- 读取硬件特征 ---")
    hw_fake = parse_csv_to_pseudo_image(hw_file, grid_h=496, grid_w=432, channels=64, drop_log_path="hw_dropped_log.csv")
