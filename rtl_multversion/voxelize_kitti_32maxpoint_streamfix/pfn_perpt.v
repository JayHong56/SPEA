module pfn_layer #(
    ...
    parameter integer AXIS_OUT_WIDTH     = 128,
    ...
    output reg         [       AXIS_OUT_WIDTH-1:0]  m_axis_pfn_data,
    output reg                                      m_axis_pfn_last,
    output wire                                     m_axis_pfn_flush_done
);
    localparam integer TDM_FACTOR = 3;
    ...
    // PFN now streams 128-bit chunks directly.
    // Example: 64 channels * 16bit / 128bit = 8 output beats per voxel.
    localparam integer OUT_WORD_DIM = AXIS_OUT_WIDTH / PT_WIDTH;
    localparam integer OUT_NUM_PARTS = (OUT_PT_DIM + OUT_WORD_DIM - 1) / OUT_WORD_DIM;
    localparam integer OUT_PART_W = (OUT_NUM_PARTS <= 2) ? 1 : $clog2(OUT_NUM_PARTS);


    localparam integer DEQUANT_SHIFT = 23;
    localparam signed [16-1:0] DEQUANT_MUL = 16'sd109;  // 3482 = round(2^28 / 77089.47481872393)，其中 256 是量化后的小数放大倍数 (Q8.8)
...
`endif  // 1. 权重与偏置 ROM 
    ...
    // 全局流水线暂停控制 (反压逻辑核心)
    // =======================================================
    // PFN now outputs one voxel as OUT_NUM_PARTS streaming beats.
    // During the 128-bit stream, max_pool_regs are reused as the output holding
    // storage, so the MAC/max-pool pipeline must be frozen until the stream ends.
    reg stream_start_pending;
    wire pipe_ready = !m_axis_pfn_valid && !stream_start_pending;
    ...
    // 当前 voxel 已经接收的有效点数。当前点进入时的点数为 cur_voxel_pt_cnt + 1。
    // 该计数用于在输出时判断是否存在 padding zero 点。
    ...
    always @(*) begin
        ...
        // phase 1/2/3 使用锁存的 latched_pfe_data。
    ...
    // =======================================================
    // 级联流水线 Stage 4: Max Pool 与 128-bit 流式输出
    // =======================================================
    // pad_max_regs removed:
    //   Old design kept max(max_valid_points, ReLU(bias)) for every channel in
    //   parallel during every phase. That duplicated 64-channel max/mux logic.
    //   New design keeps only valid-point max_pool_regs. At output time, each
    //   128-bit beat contains only OUT_WORD_DIM channels, so padding baseline is
    //   applied only to those channels being streamed.
    ...
    reg [OUT_PART_W-1:0] out_part_idx;
    reg                  out_has_padding;

    reg                  pending_has_padding;
    reg signed [COORD_WIDTH-1:0] pending_voxel_x;
    reg signed [COORD_WIDTH-1:0] pending_voxel_y;
...
`endif
    ...
    function [PT_WIDTH-1:0] final_channel_value;
        input integer ch_idx;
        input         use_padding;
        begin
            if (use_padding && (bias_relu_rom[ch_idx] > max_pool_regs[ch_idx])) begin
                final_channel_value = bias_relu_rom[ch_idx];
            end else begin
                final_channel_value = max_pool_regs[ch_idx];
            end
        end
    endfunction

    function [AXIS_OUT_WIDTH-1:0] pack_stream_word;
        input [OUT_PART_W-1:0] part_idx;
        input                  use_padding;
        integer                out_lane;
        integer                ch_idx;
        begin
            pack_stream_word = {AXIS_OUT_WIDTH{1'b0}};
            for (out_lane = 0; out_lane < OUT_WORD_DIM; out_lane = out_lane + 1) begin
                ch_idx = part_idx * OUT_WORD_DIM + out_lane;
                if (ch_idx < OUT_PT_DIM) begin
                    pack_stream_word[out_lane*PT_WIDTH+:PT_WIDTH] = final_channel_value(ch_idx, use_padding);
                end
            end
        end
    endfunction
    ...
    integer oc;
    integer oc_idx;
    wire out_fire = m_axis_pfn_valid && m_axis_pfn_tready;
    wire [OUT_PART_W-1:0] out_next_part_idx = out_part_idx + {{(OUT_PART_W - 1) {1'b0}}, 1'b1};

    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            m_axis_pfn_valid       <= 1'b0;
            m_axis_pfn_last        <= 1'b0;
            m_axis_pfn_data        <= {AXIS_OUT_WIDTH{1'b0}};
            m_axis_pfn_voxel_x     <= {COORD_WIDTH{1'b0}};
            m_axis_pfn_voxel_y     <= {COORD_WIDTH{1'b0}};
            out_part_idx           <= {OUT_PART_W{1'b0}};
            out_has_padding        <= 1'b0;
            pending_has_padding    <= 1'b0;
            pending_voxel_x        <= {COORD_WIDTH{1'b0}};
            pending_voxel_y        <= {COORD_WIDTH{1'b0}};
            stream_start_pending   <= 1'b0;
            ...
            // --------------------------------------------------
            // 1) One cycle after the last phase of a voxel, start streaming
            //    channel 0~7. This one-cycle delay ensures the last phase's
            //    max_pool_regs updates have already landed before any chunk is
            //    selected for output.
            // --------------------------------------------------
            if (stream_start_pending) begin
                stream_start_pending <= 1'b0;
                m_axis_pfn_valid     <= 1'b1;
                m_axis_pfn_last      <= (OUT_NUM_PARTS == 1);
                m_axis_pfn_data      <= pack_stream_word({OUT_PART_W{1'b0}}, pending_has_padding);
                m_axis_pfn_voxel_x   <= pending_voxel_x;
                m_axis_pfn_voxel_y   <= pending_voxel_y;
                out_part_idx         <= {OUT_PART_W{1'b0}};
                out_has_padding      <= pending_has_padding;

            // --------------------------------------------------
            // 2) Stream one 128-bit beat per handshake. max_pool_regs are the
            //    output holding storage; no 1024-bit m_axis_pfn_data and no
            //    external width_converter data_buf are needed.
            // --------------------------------------------------
            end else if (m_axis_pfn_valid) begin
                if (m_axis_pfn_tready) begin
                    if (m_axis_pfn_last) begin
                        m_axis_pfn_valid <= 1'b0;
                        m_axis_pfn_last  <= 1'b0;
                        out_part_idx     <= {OUT_PART_W{1'b0}};

                        // Release max-pool storage for the next voxel only
                        // after the last 128-bit beat has been accepted.
                        for (oc = 0; oc < OUT_PT_DIM; oc = oc + 1) begin
                            max_pool_regs[oc] <= {PT_WIDTH{1'b0}};
                        end
                    end else begin
                        out_part_idx      <= out_next_part_idx;
                        m_axis_pfn_data   <= pack_stream_word(out_next_part_idx, out_has_padding);
                        m_axis_pfn_last   <= (out_next_part_idx == (OUT_NUM_PARTS - 1));
                    end
                end

            // --------------------------------------------------
            // 3) Normal MAC/max-pool update path. This path runs only when no
            //    output stream is active/pending.
            // --------------------------------------------------
            end else begin
                if (st3_valid) begin
                    if ((st3_phase == LAST_PHASE[PHASE_W-1:0]) && st3_last) begin
                        // Last phase of the last point: update final chunk,
                        // then request 128-bit streaming from next cycle.
                        ...
                        pending_has_padding  <= has_padding_point;
                        pending_voxel_x      <= st3_voxel_x;
                        pending_voxel_y      <= st3_voxel_y;
                        stream_start_pending <= 1'b1;

                    end else begin
                        // Ordinary phase: update only the phase's chunk.
                        for (oc = 0; oc < OUT_CHUNK_DIM; oc = oc + 1) begin
    ...
    wire pfn_empty = (phase == {PHASE_W{1'b0}}) && !st2_valid && !st3_valid && !stream_start_pending && !m_axis_pfn_valid;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
...
`ifndef SYNTHESIS
    ...
    initial begin
        ...
        if (csv_fd != 0) begin
            $fwrite(csv_fd, "time,voxel_x,voxel_y,pt_cnt,part,last");
            for (out_i = 0; out_i < OUT_WORD_DIM; out_i = out_i + 1) begin
                $fwrite(csv_fd, ",dim_in_beat%0d", out_i);
            end
            $fwrite(csv_fd, "\n");
        end
    ...
    always @(posedge clk) begin
        if (rst_n && m_axis_pfn_valid && m_axis_pfn_tready) begin
            if (csv_fd != 0) begin
                $fwrite(csv_fd, "%0t,%0d,%0d,%0d,%0d,%0d", $time, m_axis_pfn_voxel_x, m_axis_pfn_voxel_y, out_pt_cnt, out_part_idx, m_axis_pfn_last);
                for (out_i = 0; out_i < OUT_WORD_DIM; out_i = out_i + 1) begin
