...
// ---------------------------------------------------------------
// A ready-to-use wrapper for clearing through ONE AXI DataMover S2MM
// channel, then returning that same DataMover to the normal scatter path.
//
// Put this module between your existing lane0 scatter/data path and the
// DataMover S2MM ports.
//
// Normal mode:
//   normal_cmd_* + normal_axis_* -> DataMover S2MM
// Clear mode:
//   s2mm_clear_writer zero command/data -> DataMover S2MM
//
// Connect normal_axis_* to the same PFN width-converter lane that originally
// went to this DataMover. Connect normal_cmd_* to the voxel_scatter_addr_gen
// command for that lane.
// ================================================================
module s2mm_clear_lane0_adapter #(
    ...
    // Existing normal S2MM command path, usually from voxel_scatter_addr_gen
    input  wire                         normal_cmd_tvalid,
    output wire                         normal_cmd_tready,
    input  wire [                 71:0] normal_cmd_tdata,
    // Existing normal S2MM data path, usually from one pfn_width_converter lane
    ...
    // Normal S2MM status is consumed when not clearing, but now we also
    // count it so the frame boundary can wait for true output drain.
