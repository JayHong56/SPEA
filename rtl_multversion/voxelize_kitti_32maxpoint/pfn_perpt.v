...
`endif  // 1. 权重与偏置 ROM 
    ...
    // =======================================================
    // 只保留 valid-point 的 max-pool 结果。
    // padding zero 对应的输出是 ReLU(bias)，不再为 padding baseline 维护额外 64 通道寄存器，
    // 而是在最后输出整包 PFN feature 时按需比较一次。
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (st3_valid) begin
                ...
                // 则本拍完成最后 phase 的 max，并输出整个 PFN feature。
                // --------------------------------------------------
                if ((st3_phase == LAST_PHASE[PHASE_W-1:0]) && st3_last) begin
                    ...
                    // 如果该 voxel 有 padding zero 点，最终输出时才额外和 ReLU(bias) 比一次，
                    ...
                    // --------------------------------------------------
                    // 普通情况：只更新当前 phase 对应的 OUT_CHUNK_DIM 个通道。
                    // padding baseline 只在最终输出时比较。
                    // --------------------------------------------------
                    for (oc = 0; oc < OUT_CHUNK_DIM; oc = oc + 1) begin
