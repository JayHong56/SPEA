module voxel_coor_pipe #(
    ...
    parameter integer BRAM_ADDR_WIDTH_PFE = 12,  // PFE cache row address width
    ...
    parameter integer VN_WIDTH = 7,  // 点编号宽度
    parameter [23:0] MAX_VOXEL_NUM = 24'd100,  // 每逻辑体素最多 100 点
    parameter [23:0] ENTRY_POINT_CAP = 24'd32,  // 每个物理 hash entry 存 32 点
    ...
    localparam integer EXPEND_VOXEL_ROW = (ENTRY_POINT_CAP + POINTS_PER_ROW - 1) / POINTS_PER_ROW;  // ceil(32/8)=4
    localparam integer RAW_PT_WIDTH = 4 * PT_WIDTH_FLOAT32;  // 128 bit
    localparam [VN_WIDTH-1:0] MAX_VOXEL_POINTS = ENTRY_POINT_CAP[VN_WIDTH-1:0];
    // BRAM clk/rst
...
`endif
    ...
    hash_bucket_table #(
        ...
        .out_point_number    (hash_out_point_number),  // segment-local 1-32
    ...
    wire hash_point_number_valid = hash_out_valid &&
                                    (hash_out_point_number <= MAX_VOXEL_POINTS) &&
                                    (hash_out_point_number != {VN_WIDTH{1'b0}});
    wire [BRAM_ADDR_WIDTH-1:0] bram_write_addr_sel = bram_row_addr;
    wire [$clog2(POINTS_PER_ROW)-1:0] bram_slot_idx_sel = bram_slot_idx;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            bram_voxelpoint_wr   <= 1'b0;
            bram_voxelpoint_bwen <= {BRAM_DATA_WIDTH / BYTE_WIDTH{1'b0}};
            // pproc_d3 与 hash_out_* 对齐；hash_out_valid 屏蔽 table full / drop。
            if (pproc_vld_d3 && hash_out_valid) begin
