module pfn_layer #(
    ...
    localparam integer OUT_CHUNK_DIM = OUT_PT_DIM / TDM_FACTOR;  // 64/4 = 16
...
`endif  // 1. 权重与偏置 ROM 
    ...
    always @(*) begin
        for (j_2 = 0; j_2 < OUT_CHUNK_DIM; j_2 = j_2 + 1) begin
            dequant_temp[j_2] = st3_mac_tree[j_2] * 779;
            if (st3_mac_tree[j_2] < 0) begin
                relu_comb[j_2] = 0;
            end else begin
                relu_comb[j_2] = dequant_temp[j_2][PT_WIDTH+20-1 : 20];
            end
        end
    end
    // =======================================================
        ...
        if (!rst_n) begin
            ...
            if (st3_valid) begin
                ...
                if ((st3_phase == LAST_PHASE[PHASE_W-1:0]) && st3_last) begin
                    ...
                    // 先输出前 36 维：它们已经在之前 phase 中更新到了 max_pool_regs
                    ...
                    // 输出最后 12 维：
                    // 当前 relu_comb 还没写入 max_pool_regs，所以这里要现场比较
                    for (oc = 0; oc < OUT_CHUNK_DIM; oc = oc + 1) begin
                        oc_idx = LAST_CHUNK_BASE + oc;

                        m_axis_pfn_data[oc_idx*PT_WIDTH+:PT_WIDTH] <=
                        (relu_comb[oc] > max_pool_regs[oc_idx]) ?
                        relu_comb[oc] :
                        max_pool_regs[oc_idx];
        ...
        if (rst_n && s_axis_pfe_valid && s_axis_pfe_tready) begin
            ...
            if (pfe_csv_fd != 0) begin
                ...
                for (pfe_i = EXPAND_PT_DIM - 1; pfe_i >= 0; pfe_i = pfe_i - 1) begin
                    ...
                    end else if (pfe_i == 2 || pfe_i == 6) begin
                        // z, z-mean: Q4.12
