module pfe #(
    ...
    parameter integer BRAM_DATA_WIDTH = 468,  // 72bit * 7 points per row
    ...
    // BRAM_B 中每个 voxel 固定保留 3 行；PFE 根据 point_num 只读取真正有效的 1/2/3 行。
    localparam integer EXPAND_VOXEL_ROW = 3;
    localparam [VN_WIDTH-1:0] POINTS_PER_ROW_V = POINTS_PER_ROW;
    localparam [VN_WIDTH-1:0] TWO_POINTS_PER_ROW_V = 2 * POINTS_PER_ROW;

    // BRAM point format: {x_q88[19:0], y_q88[19:0], z_q412[15:0], intensity_q115[15:0]} = 72 bit.
    // PFN interface remains 9 * 16 bit. x/y derived features are converted to signed Q1.15.
    localparam integer PT_WIDTH_XY = 16;
    ...
    // 支持每个 voxel 读取 1/2/3 行。
    reg                            more_row_pending;
    reg  [                    1:0] next_row_idx;
    reg  [                    1:0] st1_total_rows;
    reg  [                    2:0] req_phase;
    reg                            req_valid;
    ...
    wire [1:0] axis_need_rows = (axis_point_num_indicator <= POINTS_PER_ROW_V)     ? 2'd1 :
                                (axis_point_num_indicator <= TWO_POINTS_PER_ROW_V) ? 2'd2 :
                                                                                     2'd3;
    ...
    wire                           issue_last = issue_row0 ? (axis_need_rows == 2'd1) :
                                                            (next_row_idx == (st1_total_rows - 1'b1));
    ...
    // point_num <= 20 : 处理 row0 + row1，row1 结束后 st2_valid
    // point_num >  20 : 处理 row0 + row1 + row2，row2 结束后 st2_valid
    // ========================================================================
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            sum_x_acc      <= {SUM_XY_WIDTH{1'b0}};
            sum_y_acc      <= {SUM_XY_WIDTH{1'b0}};
            sum_z_acc      <= {SUM_Z_WIDTH{1'b0}};

            st2_valid      <= 1'b0;
            st2_done_pnum  <= {VN_WIDTH{1'b0}};
            st2_done_sum_x <= {SUM_XY_WIDTH{1'b0}};
            st2_done_sum_y <= {SUM_XY_WIDTH{1'b0}};
            st2_done_sum_z <= {SUM_Z_WIDTH{1'b0}};
    ...
    // Stage 2 -> Stage 3 完成条件：当前 voxel 的 1/2/3 行数据都读完
    reg  st3_valid;
    reg  st4_valid;
    wire st4_push;
    ...
    wire signed [PT_WIDTH_XY-1:0] meanx_center_q88;
    wire signed [PT_WIDTH_XY-1:0] meany_center_q88;
    wire signed [PT_WIDTH_Z-1:0] meanz_center_q412;

    assign meanx_center_q88 = temp_mult_x[16-PT_WIDTH_F_XY+PT_WIDTH_XY-1:16-PT_WIDTH_F_XY];
    assign meany_center_q88 = temp_mult_y[16-PT_WIDTH_F_XY+PT_WIDTH_XY-1:16-PT_WIDTH_F_XY];
    ...
    wire signed [PT_WIDTH_XY-1:0] vx_center_q88;
    wire signed [PT_WIDTH_XY-1:0] vy_center_q88;

    assign vx_center_q88 = temp_sub_voxel_x[16-PT_WIDTH_F_XY+PT_WIDTH_XY-1:16-PT_WIDTH_F_XY];
    assign vy_center_q88 = temp_sub_voxel_y[16-PT_WIDTH_F_XY+PT_WIDTH_XY-1:16-PT_WIDTH_F_XY];
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (st3_to_st4) begin
                ...
                // QI.F
                st4_mean_x   <= meanx_center_q88;
                st4_mean_y   <= meany_center_q88;
                st4_mean_z   <= meanz_center_q412;

                st4_vx_fixxy <= vx_center_q88;
                st4_vy_fixxy <= vy_center_q88;
    ...
    // FIFO 内部只存 7 维，输出到 PFN 时再复制 x/y_vcenter 扩成 9 维。
    // store7 = {x_vcenter, y_vcenter, z, intensity, x_cluster, y_cluster, z_cluster}
    // out9   = {x_vcenter, y_vcenter, z, intensity, x_cluster, y_cluster, z_cluster, x_vcenter, y_vcenter}
    localparam PFE_FIFO_DIM = 7;
    localparam PT_VEC_FIFO_W = PFE_FIFO_DIM * PT_WIDTH;       // 7 * 16 = 112 bits
    localparam PT_VEC_W      = EXPAND_PT_DIM * PT_WIDTH;      // 9 * 16 = 144 bits, PFN interface
    localparam VOX_VEC_W     = MAX_VOXEL_NUM * PT_VEC_FIFO_W; // 20 * 112 = 2240 bits

    function [PT_VEC_W-1:0] expand7_to_9;
        input [PT_VEC_FIFO_W-1:0] in7;
        ...
        begin
            x_vcenter    = in7[PT_VEC_FIFO_W-1-0*PT_WIDTH -: PT_WIDTH];
            y_vcenter    = in7[PT_VEC_FIFO_W-1-1*PT_WIDTH -: PT_WIDTH];
            z_raw        = in7[PT_VEC_FIFO_W-1-2*PT_WIDTH -: PT_WIDTH];
            intensity_raw= in7[PT_VEC_FIFO_W-1-3*PT_WIDTH -: PT_WIDTH];
            x_cluster    = in7[PT_VEC_FIFO_W-1-4*PT_WIDTH -: PT_WIDTH];
            y_cluster    = in7[PT_VEC_FIFO_W-1-5*PT_WIDTH -: PT_WIDTH];
            z_cluster    = in7[PT_VEC_FIFO_W-1-6*PT_WIDTH -: PT_WIDTH];
            expand7_to_9 = {x_vcenter, y_vcenter, z_raw, intensity_raw,
                            x_cluster, y_cluster, z_cluster, x_vcenter, y_vcenter};
        end
    endfunction
    ...
    // 每个点预计算差值：x/y 内部 Q8.12，送 PFN 前转成 Q1.15。
    wire signed [PT_WIDTH_XY-1:0] sub_x_mean_q88       [0:MAX_VOXEL_NUM-1];
    wire signed [PT_WIDTH_XY-1:0] sub_y_mean_q88       [0:MAX_VOXEL_NUM-1];
    wire signed [PT_WIDTH_XY-1:0] sub_x_vcenter_q88    [0:MAX_VOXEL_NUM-1];
    wire signed [PT_WIDTH_XY-1:0] sub_y_vcenter_q88    [0:MAX_VOXEL_NUM-1];
    ...
    generate
        for (gi = 0; gi < MAX_VOXEL_NUM; gi = gi + 1) begin : GEN_SUB_ALL
            ...
            fxp_addsub #(
                ...
                .inb     (st4_vy_fixxy),
                .sub     (1'b1),
                .out     (sub_y_vcenter_q88[gi]),
                .overflow(sub_y_vcenter_ovf[gi])
            );

            assign sub_x_mean[gi]    = sub_x_mean_q88[gi];
            assign sub_y_mean[gi]    = sub_y_mean_q88[gi];
            assign sub_x_vcenter[gi] = sub_x_vcenter_q88[gi];
            assign sub_y_vcenter[gi] = sub_y_vcenter_q88[gi];

        end
    endgenerate

    // 【核心改造 3】使用纯组合逻辑拼接 FIFO 内部 7 维数据位
    reg [VOX_VEC_W-1:0] pack_st4_voxel;
    integer pi;
    always @(*) begin
        for (pi = 0; pi < MAX_VOXEL_NUM; pi = pi + 1) begin
            if (pi < st4_pnum) begin
                pack_st4_voxel[pi*PT_VEC_FIFO_W+:PT_VEC_FIFO_W] = {
                    sub_x_vcenter[pi],
                    sub_y_vcenter[pi],
                ...
                pack_st4_voxel[pi*PT_VEC_FIFO_W+:PT_VEC_FIFO_W] = {PT_VEC_FIFO_W{1'b0}};
            end
        end
    end
    ...
    always @(posedge clk) begin
        if (st4_push) begin
            ep_fifo_data[fifo_wr_ptr]    <= pack_st4_voxel;  // 一次性写入 2240 bit 单一地址
    ...
    // 【核心改造 6】将当前要读取的 2240-bit 宽字整条拉出，做组合逻辑切片
    ...
    if (!rst_n) begin
        ...
        if (m_axis_pfe_valid && m_axis_pfe_last) begin
            ...
            if (has_next_after_pop) begin
                ...
                m_axis_pfe_data    <= expand7_to_9(next_voxel_data[0+:PT_VEC_FIFO_W]);
                point_cnt          <= {VN_WIDTH{1'b0}};
        ...
        // ================================================================
            ...
            m_axis_pfe_data <= expand7_to_9(rd_voxel_data[next_point_cnt*PT_VEC_FIFO_W+:PT_VEC_FIFO_W]);
        ...
        // ================================================================
            ...
            if (!fifo_empty) begin
                m_axis_pfe_valid   <= 1'b1;
                m_axis_pfe_data    <= expand7_to_9(rd_voxel_data[0+:PT_VEC_FIFO_W]);
                point_cnt          <= {VN_WIDTH{1'b0}};
                ...
                m_axis_pfe_valid   <= 1'b1;
                m_axis_pfe_data    <= expand7_to_9(pack_st4_voxel[0+:PT_VEC_FIFO_W]);
                point_cnt          <= {VN_WIDTH{1'b0}};
