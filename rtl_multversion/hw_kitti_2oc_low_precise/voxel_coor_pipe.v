module voxel_coor_pipe #(
    parameter integer BRAM_DATA_WIDTH = 468,  // 64* 8
    parameter integer BRAM_ADDR_WIDTH = 10,  // 256 pillar * 4brams
    ...
    output wire [BRAM_DATA_WIDTH/BYTE_WIDTH-1:0] bram_expire_bwen_b,
    ...
    localparam integer PT_WIDTH_PER = 2 * PT_WIDTH_XY + 2 * PT_WIDTH;  // 64 bit 每点
    localparam integer PT_WIDTH_FLOAT32 = 32;
    localparam integer VOXEL_WIDTH = 11;  // NOTE 根据伪图尺寸设定

    localparam integer POINTS_PER_ROW = BRAM_DATA_WIDTH / PT_WIDTH_PER;  // 468/64 = 7
...
`endif
    ...
    wire signed [PT_WIDTH_XY-1:0] point_x_fix16_data;
    wire signed [PT_WIDTH_XY-1:0] point_y_fix16_data;
    ...
    float_to_fixed_data #(
        ...
        .true_fixed_value(point_x_fix16_data),
        .fixed_sign      ()
    );
    ...
    float_to_fixed_data #(
        ...
        .true_fixed_value(point_y_fix16_data),
        .fixed_sign      ()
    );
    ...
    assign point_abs_x = point_x_fix16_data[PT_WIDTH_XY-1] ? (~point_x_fix16_data + 1'b1) : point_x_fix16_data;
    assign point_abs_y = point_y_fix16_data[PT_WIDTH_XY-1] ? (~point_y_fix16_data + 1'b1) : point_y_fix16_data;
    ...
    always @(*) begin
        if (!rst_n) begin
            ...
            //     keep_point <= 1'b0;
        end else if (^point_x_fix16_data === 1'bx || point_raw_r == {RAW_PT_WIDTH{1'b0}}) begin
            keep_point = 1'b0;

        end else if (point_x_fix16_data[PT_WIDTH_XY-1]) begin
            keep_point = 1'b0;
    ...
    localparam signed [PT_WIDTH_XY-1:0] THRESHOLD_BOUDARY_Y_CEIL_FIX = 16'sh27af;  // 39.68m 但加了一个 LSB
    // ------------------------------------------------------------
    ...
    assign voxel_x_scaled = voxel_x_mul25 >>> 10;
    ...
    assign voxel_y_shift_q12 = $signed(
        {point_y_fix16_data[PT_WIDTH_XY-1], point_y_fix16_data}
    ) + $signed(
    ...
    assign voxel_y_scaled = voxel_y_mul25 >>> 10;
    ...
    assign single_point_proc = {point_x_fix16_data, point_y_fix16_data, point_z_fix16, point_intensity_fix16};
...
`ifndef SYNTHESIS
    ...
    wire test_coor = (point_y_fix16_data == 20'h15c28);
