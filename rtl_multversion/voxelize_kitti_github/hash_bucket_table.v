module hash_bucket_table #(
    ...
    parameter         TABLE_SIZE          = BUCKETS * (1'b1 << BUCKET_SLOT_WIDTH),  
    ...
    output reg [ADDR_WIDTH-1:0] out_idx,  
    ...
    wire expire_blocking;
    wire shadow_read_en = !expire_blocking;
    hash_bucket_array_shadow #(
        ...
        .en0_a   (shadow_read_en),
        .en1_a   (shadow_read_en),
        .en2_a   (shadow_read_en),
        .en3_a   (shadow_read_en),
    ...
    wire action_true_full = action_full && (probe_offset == {BUCKET_AW{1'b1}});  
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            is_probing   <= 1'b0;
            probe_wait   <= 1'b0;
    ...
    hash_expire_manager #(
        ...
        .expire_blocking     (expire_blocking)
    );
    ...
    reg [ADDR_WIDTH-1:0] cam_mapped_idx[0:CAM_DEPTH-1];  
    reg [VN_WIDTH-1:0] cam_pn[0:CAM_DEPTH-1];  
    ...
    always @(*) begin
        ...
        for (j = 0; j < CAM_DEPTH; j = j + 1) begin  
...
`endif
    ...
    assign en0_a = (kill_stall | action_full | hash_stall) ? 1'b0 : 1'b1;  
...
`ifndef SYNTHESIS
    ...
    wire monitor_kill_valid = action_kill && (kill_wdata[ENTRY_WIDTH-1-:2] == ST_TOMB);
    hash_load_monitor #(
        .BUCKETS   (BUCKETS),
        .BUCKET_AW (BUCKET_AW),
        .MAX_SLOTS (4),
        ...
        .alloc_bucket(free_eval_bucket),

        
        .kill_valid(monitor_kill_valid),
        .kill_addr (kill_addr),
