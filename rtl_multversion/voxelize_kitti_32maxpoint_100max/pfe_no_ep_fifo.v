...
module pfe_no_ep_fifo #(
    ...
    // BRAM_B 中每个 voxel 固定保留 EXPAND_VOXEL_ROW 行；PFE 根据 point_num 只读取真正有效的 1/2/3/4 行。
    ...
    // =========================================================================
    // Stage 5: FIFO removed
    // =========================================================================
    // 原 FIFO 中只存 7 维；取消 FIFO 后仍保留 7维打包，输出到 PFN 时再复制
    // x/y_vcenter 扩成 9 维。
    ...
    localparam PFE_STORE_DIM = 7;
    localparam PT_VEC_STORE_W = PFE_STORE_DIM * PT_WIDTH;  // 7 * 16 = 112 bits
    localparam PT_VEC_W = EXPAND_PT_DIM * PT_WIDTH;      // 9 * 16 = 144 bits, PFN interface
    localparam VOX_VEC_W = MAX_VOXEL_NUM * PT_VEC_STORE_W; // 32 * 112 = 3584 bits

    function [PT_VEC_W-1:0] expand7_to_9;
        input [PT_VEC_STORE_W-1:0] in7;
        ...
        begin
            x_vcenter = in7[PT_VEC_STORE_W-1-0*PT_WIDTH-:PT_WIDTH];
            y_vcenter = in7[PT_VEC_STORE_W-1-1*PT_WIDTH-:PT_WIDTH];
            z_raw = in7[PT_VEC_STORE_W-1-2*PT_WIDTH-:PT_WIDTH];
            intensity_raw = in7[PT_VEC_STORE_W-1-3*PT_WIDTH-:PT_WIDTH];
            x_cluster = in7[PT_VEC_STORE_W-1-4*PT_WIDTH-:PT_WIDTH];
            y_cluster = in7[PT_VEC_STORE_W-1-5*PT_WIDTH-:PT_WIDTH];
            z_cluster = in7[PT_VEC_STORE_W-1-6*PT_WIDTH-:PT_WIDTH];
            expand7_to_9 = {
    ...
    always @(*) begin
        for (pi = 0; pi < MAX_VOXEL_NUM; pi = pi + 1) begin
            if (pi < st4_pnum) begin
                ...
                pack_st4_voxel[pi*PT_VEC_STORE_W+:PT_VEC_STORE_W] = {PT_VEC_STORE_W{1'b0}};
            end
        end
    end

    // Stage4 已经直接作为输出 holding buffer，因此这里不再写入 FIFO。
        ...
        if (!rst_n) begin
            ...
            if (m_axis_pfe_valid) begin
                ...
                if (m_axis_pfe_last) begin
                    ...
                    m_axis_pfe_data    <= expand7_to_9(pack_st4_voxel[next_point_cnt*PT_VEC_STORE_W+:PT_VEC_STORE_W]);
                ...
                if (st4_valid) begin
                    ...
                    m_axis_pfe_data    <= expand7_to_9(pack_st4_voxel[0+:PT_VEC_STORE_W]);
                    point_cnt          <= {VN_WIDTH{1'b0}};
                    m_axis_pfe_voxel_x <= st4_voxel_x;
                    m_axis_pfe_voxel_y <= st4_voxel_y;
