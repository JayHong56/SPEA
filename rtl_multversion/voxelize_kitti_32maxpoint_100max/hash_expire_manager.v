module hash_expire_manager #(
    ...
    input wire force_expire_valid,
    output wire force_expire_ready,
    input wire [ADDR_WIDTH-1:0] force_expire_addr,
    ...
    output wire signed [COORD_WIDTH-1:0] kill_key_x,
    output wire signed [COORD_WIDTH-1:0] kill_key_y,
    output wire [ADDR_WIDTH-1:0] kill_addr_flat,
...
`endif
    ...
    // 动态选择：Flush 强制扫描优先，其次是外部强制过期候选，最后是正常 DQ 候选。
    wire force_run = force_expire_valid && force_expire_ready;
    wire eff_cand_valid = flush_run ? 1'b1 : force_run ? 1'b1 : cand_valid;
    wire [ADDR_WIDTH-1:0] eff_cand_addr = flush_run ? flush_cnt[ADDR_WIDTH-1:0] :
                                          force_run ? force_expire_addr : cand_addr;
    wire eff_force_expire = flush_run | force_run;
    // 流水线推进条件：正常的写提交脉冲 OR 正在执行强制扫描
    // wire pipe_advance = write_commit_delayed | flush_run;
    wire expire_block;
    assign force_expire_ready = !flushing && !hash_stall && !expire_block;
    wire pipe_advance_raw = flush_run | force_run | cand_valid | s0_valid | s1_valid;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            s0_time         <= time_now;
            s0_force_expire <= eff_force_expire;
    ...
    // 504-bit 一行，每个点 4 * 16 = 64-bit，所以一行 7.875 点
    localparam integer POINTS_PER_ROW = BRAM_DATA_WIDTH / PT_WIDTH_PER;  // 576/72 = 8
    // BRAM_A / BRAM_B 中每个 voxel 逻辑上固定保留 3 行
    // 注意：voxel_coor_pipe/hash table 写 BRAM_A 时也必须使用相同的 EXPEND_VOXEL_ROW。
    ...
    assign kill_key_x = r_key_x;
    assign kill_key_y = r_key_y;
    assign kill_addr_flat = s1_addr;
    ...
    // 根据有效点数决定需要搬运 1/2/3/4 行。BRAM_B 固定为每个 voxel 预留 EXPEND_VOXEL_ROW 行。
    ...
    reg  [                    2:0] fsm_copy_rows;  // 当前 task 实际需要复制的行数：1/2/3/4
    reg  [                    1:0] fsm_next_row_idx;  // ST_COPY 中下一次要发起读取的 row index
    ...
    wire                           expire_accept = s1_valid && expired && task_can_push;
    wire                           task_push = expire_accept;

    assign expire_block = s1_valid && expired && !task_can_push;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (copy_st)
                ...
                // 后续是否继续读 row1/row2/row3 由 task_copy_rows 决定。
                // -------------------------------------------------------------
                ST_IDLE: begin
                ...
                // -------------------------------------------------------------
                // ST_COPY:
                // 循环复制当前 voxel 的 row1 / row2。
                // 如果当前要发的是最后一行，则必须先确认 notify_fifo 有空间，
                // 防止最后一行写完后无法通知 PFE。
                // -------------------------------------------------------------
                ST_COPY: begin
                    if ((fsm_next_row_idx != (fsm_copy_rows - 1'b1)) || notify_has_room) begin
                        ...
                        // 当前下一行就是最后一行，但 notify FIFO 没空间，暂停读请求。
                        copy_st <= ST_COPY;
                    end
                end
