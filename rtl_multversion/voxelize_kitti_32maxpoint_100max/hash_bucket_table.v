module hash_bucket_table #(
    ...
    parameter integer BRAM_ADDR_WIDTH_PFE = 8,                                      // PFE cache row address
    ...
    wire action_cam_full_full;
    wire regular_alloc_need;
    wire spill_alloc_need;
    wire action_kill;
    wire action_full;
        ...
        if (!rst_n) begin
            ...
            if (kill_stall | hash_stall) begin
                ...
                if (probe_wait) begin
                    ...
                    if (action_free || action_cam_full_full) begin
                        // 探测成功，退出探测模式
                        is_probing   <= 1'b0;
                        probe_offset <= {BUCKET_AW{1'b0}};
                        probe_spill_mode <= 1'b0;
    ...
    wire force_expire_valid;
    wire force_expire_ready;
    wire [ADDR_WIDTH-1:0] force_expire_addr;
    ...
    hash_expire_manager #(
        ...
        .force_expire_valid  (force_expire_valid),
        .force_expire_ready  (force_expire_ready),
        .force_expire_addr   (force_expire_addr),
    ...
    // cam_full 记录同一个逻辑 pillar 因单个物理 slot 点数满而分配出的外溢 slot。
    // 多个 cam_full entry 可以属于同一个 key；只有 tail entry 继续接收新点。
    reg cam_full_valid[0:CAM_DEPTH-1];
    reg cam_full_tail[0:CAM_DEPTH-1];
    reg signed [COORD_WIDTH-1:0] cam_full_kx[0:CAM_DEPTH-1];
    reg signed [COORD_WIDTH-1:0] cam_full_ky[0:CAM_DEPTH-1];
    reg [ADDR_WIDTH-1:0] cam_full_mapped_idx[0:CAM_DEPTH-1];
    reg [ADDR_WIDTH-1:0] cam_full_root_idx[0:CAM_DEPTH-1];
    reg [VN_WIDTH-1:0] cam_full_pn[0:CAM_DEPTH-1];
    reg cam_full_force_pending[0:CAM_DEPTH-1];
    reg root_force_pending;
    reg [ADDR_WIDTH-1:0] root_force_addr;

    reg cam_full_hit;
    reg [CAM_AW-1:0] cam_full_hit_idx;
    reg cam_full_free_found;
    reg [CAM_AW-1:0] cam_full_free_idx;
    reg force_cam_pending_found;
    reg [CAM_AW-1:0] force_cam_pending_idx;
    ...
    always @(*) begin
        ...
        for (cf_i = 0; cf_i < CAM_DEPTH; cf_i = cf_i + 1) begin
            if (cam_full_valid[cf_i]) begin
                if (cam_full_tail[cf_i] && (cam_full_kx[cf_i] == s1_x) && (cam_full_ky[cf_i] == s1_y)) begin
                    cam_full_hit     = 1'b1;
    ...
    integer f_i;
    always @(*) begin
        force_cam_pending_found = 1'b0;
        force_cam_pending_idx   = {CAM_AW{1'b0}};
        for (f_i = 0; f_i < CAM_DEPTH; f_i = f_i + 1) begin
            if (!force_cam_pending_found && cam_full_force_pending[f_i]) begin
                force_cam_pending_found = 1'b1;
                force_cam_pending_idx   = f_i[CAM_AW-1:0];
            end
        end
    end

    assign force_expire_valid = root_force_pending | force_cam_pending_found;
    assign force_expire_addr  = root_force_pending ? root_force_addr : cam_full_mapped_idx[force_cam_pending_idx];

    wire hit_full = hit && (pn[hit_way] == MAX_VOXEL_NUM);
    wire cam_hit_full = cam_hit && (cam_pn[cam_hit_idx] == MAX_VOXEL_NUM);
    wire cam_full_tail_full = cam_full_hit && (cam_full_pn[cam_full_hit_idx] == MAX_VOXEL_NUM);
    wire cam_full_tail_writable = cam_full_hit && !cam_full_tail_full;
    wire needs_spill_slot = (hit && hit_full) ||
                            (!hit && cam_hit && cam_hit_full) ||
                            (!hit && !cam_hit && cam_full_hit && cam_full_tail_full);
    assign regular_alloc_need = s1_valid && !probe_spill_mode && !hit && !cam_hit && !cam_full_hit;
    assign spill_alloc_need = (s1_valid && needs_spill_slot && !cam_full_tail_writable) ||
                              (is_probing && probe_spill_mode);
    wire [ADDR_WIDTH-1:0] active_primary_idx = hit ? {hit_eval_bucket, hit_way} :
                                               cam_hit ? cam_mapped_idx[cam_hit_idx] :
                                               cam_full_root_idx[cam_full_hit_idx];
    wire [ADDR_WIDTH-1:0] spill_root_idx = (is_probing && probe_spill_mode) ? probe_spill_root_idx :
                                           cam_full_hit ? cam_full_root_idx[cam_full_hit_idx] : active_primary_idx;
    ...
    assign next_bp_data = action_hit_base ? make_entry(
        ...
        s1_x,
        s1_y,
        (cam_full_pn[cam_full_hit_idx] == MAX_VOXEL_NUM) ? MAX_VOXEL_NUM : cam_full_pn[cam_full_hit_idx] + 1'b1,
        global_timer
    ...
    assign next_bp_addr = action_hit_base ? {hit_eval_bucket, hit_way} : 
                          ...
                          action_hit_cam_full ? cam_full_mapped_idx[cam_full_hit_idx] :
...
`endif
    ...
    assign kill_stall = kill_expired | kill_valid | kill_write | force_expire_valid;
    wire full_stall = (action_full | is_probing) & (!action_free) & (!action_cam_full_full);
    ...
    assign action_hit_base = s1_valid && hit && !hit_full && !kill_stall && (!probe_wait) && !hash_stall;
    assign action_hit_cam = s1_valid && (!hit) && cam_hit && !cam_hit_full && !kill_stall && (!probe_wait) && !hash_stall;
    assign action_hit_cam_full = s1_valid && cam_full_tail_writable &&
                                 ((hit && hit_full) || (!hit && cam_hit && cam_hit_full) || (!hit && !cam_hit)) &&
                                 !kill_stall && (!probe_wait) && !hash_stall;

    assign action_alloc_regular_free = regular_alloc_need && free_found && !kill_stall && (!probe_wait) && !hash_stall;
    assign action_alloc_spill_free = spill_alloc_need && free_found && cam_full_free_found && !kill_stall && (!probe_wait) && !hash_stall;
    assign action_free = action_alloc_regular_free | action_alloc_spill_free;
    assign action_cam_full_full = spill_alloc_need && free_found && !cam_full_free_found && !kill_stall && (!probe_wait) && !hash_stall;
    assign action_full = (regular_alloc_need || spill_alloc_need) && (!free_found) && !kill_stall && (!probe_wait) && !hash_stall;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            table_full               <= 1'b0;
            rr_ptr                   <= 2'd0;
            root_force_pending       <= 1'b0;
            root_force_addr          <= {ADDR_WIDTH{1'b0}};
            ...
            for (k = 0; k < CAM_DEPTH; k = k + 1) begin
                ...
                cam_full_tail[k] <= 1'b0;
                cam_full_force_pending[k] <= 1'b0;
            end
            ...
            if (force_expire_valid && force_expire_ready) begin
                if (root_force_pending) begin
                    root_force_pending <= 1'b0;
                end else if (force_cam_pending_found) begin
                    cam_full_force_pending[force_cam_pending_idx] <= 1'b0;
                end
            end
            ...
            if (action_hit_base) begin
                ...
                busy <= 1'b1;
                out_idx <= cam_full_mapped_idx[cam_full_hit_idx];
                shared_write_addr_bucket <= cam_full_mapped_idx[cam_full_hit_idx][ADDR_WIDTH-1 : BUCKET_SLOT_WIDTH];
                out_point_number <= (cam_full_pn[cam_full_hit_idx] == MAX_VOXEL_NUM) ?
                                    MAX_VOXEL_NUM : cam_full_pn[cam_full_hit_idx] + 1'b1;

                case (cam_full_mapped_idx[cam_full_hit_idx][BUCKET_SLOT_WIDTH-1:0])
                    2'd0: begin
                        we0_b <= 1;
                        wdata0_b <= make_entry(
                            ST_OCCU,
                ...
                endcase
                cam_full_pn[cam_full_hit_idx] <= (cam_full_pn[cam_full_hit_idx] == MAX_VOXEL_NUM) ?
                                                 MAX_VOXEL_NUM : cam_full_pn[cam_full_hit_idx] + 1'b1;
                ...
                if (action_alloc_spill_free) begin
                    for (k = 0; k < CAM_DEPTH; k = k + 1) begin
                        if (cam_full_valid[k] && (cam_full_kx[k] == s1_x) && (cam_full_ky[k] == s1_y)) begin
                            cam_full_tail[k] <= 1'b0;
                        end
                    end
                    cam_full_valid[cam_full_free_idx]      <= 1'b1;
                    cam_full_tail[cam_full_free_idx]       <= 1'b1;
                    cam_full_kx[cam_full_free_idx]         <= s1_x;
                    cam_full_ky[cam_full_free_idx]         <= s1_y;
                    cam_full_mapped_idx[cam_full_free_idx] <= {free_eval_bucket, free_way};
                    cam_full_root_idx[cam_full_free_idx]   <= spill_root_idx;
                    cam_full_pn[cam_full_free_idx]         <= {{(VN_WIDTH - 1) {1'b0}}, 1'b1};
                    cam_full_force_pending[cam_full_free_idx] <= 1'b0;
                end

            end else if (action_true_full) begin
                table_full <= 1'b1;
            end else if (action_cam_full_full) begin
                table_full <= 1'b1;
            end
            ...
            if (action_kill) begin
                ...
                for (k = 0; k < CAM_DEPTH; k = k + 1) begin
                    ...
                    if (cam_full_valid[k] && (cam_full_kx[k] == kill_key_x) && (cam_full_ky[k] == kill_key_y)) begin
                        if (cam_full_mapped_idx[k] != kill_addr) begin
                            cam_full_force_pending[k] <= 1'b1;
                        end else if (cam_full_root_idx[k] != kill_addr) begin
                            root_force_pending <= 1'b1;
                            root_force_addr    <= cam_full_root_idx[k];
                        end
                        cam_full_valid[k] <= 1'b0;
                        cam_full_tail[k]  <= 1'b0;
                    end
