module pfn_layer #(
    ...
    parameter integer MAX_VOXEL_NUM      = 20
) (
    input  wire                                     clk,
    input  wire                                     rst_n,
    ...
    localparam integer OUT_CHUNK_DIM = OUT_PT_DIM / TDM_FACTOR;  // 64/2=32 channels per phase
    localparam integer PHASE_W = 1;
    localparam integer LAST_PHASE = TDM_FACTOR - 1;
    localparam integer LAST_CHUNK_BASE = LAST_PHASE * OUT_CHUNK_DIM;  // 1*32=32
...
`ifndef SYNTHESIS
    initial begin
        if ((OUT_PT_DIM % TDM_FACTOR) != 0) begin
            $display("[ERROR][pfn_layer] TDM_FACTOR=2 requires OUT_PT_DIM to be exactly divisible. OUT_PT_DIM=%0d", OUT_PT_DIM);
            $stop;
        end
    end
`endif  // 1. 权重与偏置 ROM 
    ...
    // =======================================================
    reg [PHASE_W-1:0] phase;  // TDM_FACTOR=2: phase 0/1
    ...
    always @(*) begin
        ...
        // phase 1 使用锁存的 latched_pfe_data。
        ...
        // 2. TDM_FACTOR=2 时每个 phase 固定计算 32 个输出通道：
        // phase 0 -> 0  ~ 31
        // phase 1 -> 32 ~ 63
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // 2. 数据计算：当有有效数据时，计算 9 维 dot-product 加 bias，并打入寄存器
            if (st2_valid) begin
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (st3_valid) begin
                ...
                // 则本拍完成最后 32 维的 max，并输出整个 64 维结果。
                // --------------------------------------------------
