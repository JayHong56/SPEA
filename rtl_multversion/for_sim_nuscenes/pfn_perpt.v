module pfn_layer #(
    ...
    localparam integer OUT_CHUNK_DIM = OUT_PT_DIM / TDM_FACTOR;  // 64/2 = 32
    ...
    localparam integer DEQUANT_SHIFT = 20;
    localparam signed [31:0] DEQUANT_MUL = 32'sd994;  // 3482 = round(2^28 / 77089.47481872393)，其中 256 是量化后的小数放大倍数 (Q8.8)
...
`ifndef SYNTHESIS
    initial begin
        if (OUT_PT_DIM % TDM_FACTOR != 0) begin
            $display("[ERROR][pfn_layer] OUT_PT_DIM must be divisible by 4.");
            $stop;
        end
    end
`endif  // 1. 权重与偏置 ROM 
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            st2_voxel_pt_cnt     <= {PT_CNT_WIDTH{1'b0}};
            latched_last         <= 1'b0;
            latched_voxel_valid  <= 1'b0;
            latched_voxel_x      <= {COORD_WIDTH{1'b0}};
            latched_voxel_y      <= {COORD_WIDTH{1'b0}};
            latched_voxel_pt_cnt <= {PT_CNT_WIDTH{1'b0}};
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            st3_voxel_pt_cnt <= {PT_CNT_WIDTH{1'b0}};
    ...
    reg        [PT_WIDTH-1:0] cur_valid_max;
    reg        [PT_WIDTH-1:0] bias_pad_relu;
    reg        [PT_WIDTH-1:0] final_max;
...
`endif
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            m_axis_pfn_data    <= {OUT_PT_DIM*PT_WIDTH{1'b0}};
            ...
            if (st3_valid) begin
                ...
                // --------------------------------------------------
                // 如果当前是最后一个 phase，并且当前点是 voxel 的最后一个点，
                // 则本拍完成最后 12 维的 max，并输出整个 48 维结果。
                // --------------------------------------------------
                if ((st3_phase == LAST_PHASE[PHASE_W-1:0]) && st3_last) begin
                    ...
                    // 先输出前面已经累积在 max_pool_regs 中的通道。
                    // 如果该 voxel 有 padding zero 点，还要额外和 ReLU(bias) 比一次，
                    // 等效于软件中 padding zero 点参与 torch.max(x, dim=1)。
                    for (oc = 0; oc < LAST_CHUNK_BASE; oc = oc + 1) begin
                        cur_valid_max = max_pool_regs[oc];
                        bias_pad_relu = bias_relu_rom[oc];

                        if (has_padding_point && (bias_pad_relu > cur_valid_max)) begin
                            final_max = bias_pad_relu;
                        end else begin
                            final_max = cur_valid_max;
                        end

                        m_axis_pfn_data[oc*PT_WIDTH+:PT_WIDTH] <= final_max;
                        max_pool_regs[oc] <= 0;
                    end

                    // 输出最后一个 phase 的通道：当前 relu_comb 还没写入 max_pool_regs，
                    // 所以先比较 valid-point max，再根据需要加入 padding bias baseline。
                    for (oc = 0; oc < OUT_CHUNK_DIM; oc = oc + 1) begin
                        oc_idx = LAST_CHUNK_BASE + oc;

                        if (relu_comb[oc] > max_pool_regs[oc_idx]) begin
                            cur_valid_max = relu_comb[oc];
                        end else begin
                            cur_valid_max = max_pool_regs[oc_idx];
                        end

                        bias_pad_relu = bias_relu_rom[oc_idx];
                        ...
                        m_axis_pfn_data[oc_idx*PT_WIDTH+:PT_WIDTH] <= final_max;
                        max_pool_regs[oc_idx] <= 0;
                    end

                end else begin
                    // --------------------------------------------------
                    // 普通情况：只更新当前 phase 对应的 12 个通道
                    // --------------------------------------------------
                    for (oc = 0; oc < OUT_CHUNK_DIM; oc = oc + 1) begin
                        oc_idx = st3_phase * OUT_CHUNK_DIM + oc;

                        if (relu_comb[oc] > max_pool_regs[oc_idx]) begin
                            max_pool_regs[oc_idx] <= relu_comb[oc];
                        end
                    end
                end
            end
