module hash_expire_manager #(
    ...
    parameter integer BRAM_DATA_WIDTH = 504,  // 10 * 64
    ...
    output reg [BRAM_DATA_WIDTH/BYTE_WIDTH-1:0] bram_expire_bwen_b,
...
`endif
    ...
    // 注意：voxel_coor_pipe/hash table 写 BRAM_A 时也必须使用相同的 EXPEND_VOXEL_ROW=3。
    localparam integer EXPEND_VOXEL_ROW = 3;
    localparam integer RESERVED_VOXEL_ROW = 3;
    ...
    // 根据有效点数决定需要搬运 1/2/3 行。BRAM_B 仍然固定为每个 voxel 预留 3 行。
    wire [1:0] task_copy_rows = (task_pn <= POINTS_PER_ROW_V) ? 2'd1 : (task_pn <= TWO_POINTS_PER_ROW_V) ? 2'd2 : 2'd3;
    ...
    reg  [                    1:0] fsm_copy_rows;  // 当前 task 实际需要复制的行数：1/2/3
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (copy_st)
                ...
                // 后续是否继续读 row1/row2 由 task_copy_rows 决定。
                // -------------------------------------------------------------
                ST_IDLE: begin
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            bram_expire_bwen_b   <= {(BRAM_DATA_WIDTH / BYTE_WIDTH) {1'b0}};
            ...
            if (fsm_read_en_d1) begin
                ...
                bram_expire_bwen_b   <= {(BRAM_DATA_WIDTH / BYTE_WIDTH) {1'b1}};
            end else begin
                bram_expire_wr_b   <= 1'b0;
                bram_expire_bwen_b <= {(BRAM_DATA_WIDTH / BYTE_WIDTH) {1'b0}};
            end
